...
public class RMFurnaceTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    private RMItemHandler inventory;

    public RMFurnaceTileEntity(TileEntityType<?> tileEntityTypeIn) {
        super(tileEntityTypeIn);

        this.inventory = new RMItemHandler(2);
    }

    public RMFurnaceTileEntity() {
        this(RMTileEntityTypes.RM_FURNACE.get());
    }
    ...
    @Nullable
    private RMRecipe getRecipe(ItemStack stack) {
        if (stack == null) {
            return null;
        }
        ...
        for (IRecipe<?> iRecipe : recipes) {
            RMRecipe recipe = (RMRecipe) iRecipe;
